...
const AboutScene = () => {
  ...
  return (
    <section ref={ref} className="relative w-full min-h-screen bg-massa-paper overflow-hidden flex items-center justify-center px-6 pt-32 md:pt-40 pb-24 md:pb-32">
      ...
      <div className="relative z-10 w-full max-w-6xl grid grid-cols-1 md:grid-cols-[1.1fr_1fr] gap-8 md:gap-10 items-center">
        ...
        <motion.div className="bg-massa-surface text-massa-black border border-massa-black/10 p-10 md:p-12 shadow-[0_24px_60px_rgba(20,20,20,0.12)] flex flex-col gap-5" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={vp} transition={{ duration: 0.55, ease: EASE_POWER3, delay: 0.2 }}>
          ...
          <span className="font-mono text-sm md:text-base opacity-85 border-t border-massa-black/15 pt-3 mt-2">{t("scene14.stack")}</span>
          <div className="flex flex-wrap gap-2 pt-1">
            {stack.map((item) => (
              <span key={item} className="font-mono text-xs md:text-sm px-3 py-2 border border-massa-black/15 rounded-full bg-massa-paper">
                {item}
              </span>
            ))}
          </div>

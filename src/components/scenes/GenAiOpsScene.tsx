...
const GenAiOpsScene = () => {
  ...
  return (
    <section ref={ref} className="relative w-full min-h-screen bg-massa-red overflow-hidden pt-36 md:pt-40">
      ...
      <div className="absolute inset-0 flex items-center justify-center z-10 px-6">
        <motion.div className="bg-massa-black w-full md:w-[55vw] min-h-[55vh] flex flex-col overflow-hidden transition-transform duration-200 hover:-translate-y-2 hover:border hover:border-massa-red/60 cursor-pointer" initial={{ opacity: 0, y: 36, scale: 0.98 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={vp} transition={{ duration: 0.6, ease: EASE_POWER3, delay: 0.12 }}>
          ...
          <motion.div className="p-8 md:p-12 flex-1 flex flex-col gap-4" variants={makeStagger(0.08, 0.12)} initial="hidden" whileInView="show" viewport={vp}>
            <motion.h3 className="font-display text-massa-white text-[7vw] md:text-[3.1vw] leading-[0.9] uppercase" variants={variants.heavyReveal}>
              {content.titleKeys.map((key) => t(key)).join("\n").split("\n").map((line) => (
                <span key={line} className="block">{line}</span>
              ))}
            </motion.h3>
            <motion.p className="font-mono text-massa-white text-base opacity-85 leading-relaxed max-w-[420px]" variants={variants.subtleText}>
              {t(content.whatKey!)}
            </motion.p>
            ...
            <motion.p className="font-mono text-massa-white text-xs opacity-50 max-w-[420px] leading-relaxed" variants={variants.subtleText}>
              {t(content.descKey!)}
            </motion.p>
            <motion.div className="font-mono text-massa-white text-sm flex justify-between items-center opacity-75 mt-4 cursor-pointer" variants={variants.subtleText}>
